...
namespace _rand_cpu_
{
	...
	double default_seeds[6]{ 298193, 104959, 84736, 727366, 94727, 5928384 };

	struct RollMatrixBuff
	{
		int num;
		arma::Mat<int> A1_mat;
		arma::Mat<int> A2_mat;
		RollMatrixBuff() : num(0) { }
...
namespace cpu_rand
{
	...
	inline void rand01_naive_skip_ahead(int times, double *seed_ar)
	{

	}

	inline void rand01_skip_ahead(int times, double *seed_ar)
	{
		using namespace _rand_cpu_;
#ifdef NAIVE_SKIP
		while (times > 0)
...
#else
		if (times != _roll_matrix_buff.num)
		{
			...
			_roll_matrix_buff.A1_mat = arma::pow(A1, times);
			_roll_matrix_buff.A1_mat = arma::pow(A2, times);
		}

		arma::Col<int> Y1{ (int)seed_ar[0], (int)seed_ar[1], (int)seed_ar[2] }, Y2{ (int)seed_ar[3], (int)seed_ar[4], (int)seed_ar[5] };

		Y1 = _roll_matrix_buff.A1_mat * Y1;
		Y2 = _roll_matrix_buff.A1_mat * Y2;
