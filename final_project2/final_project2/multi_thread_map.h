...
std::vector<ReturnType> multi_map(unsigned int pool_size, size_t n_jobs, ReturnType(*func)(size_t, Args...), Args... args)
{
	...
	{
		futures[this_id] = std::async([=, &args...] { return std::make_pair(this_id, func(this_id, args...)); });
	}
	...
	while (finished_count < n_jobs)
	{
		...
		{
			if (fut.valid() && fut.wait_for(span) == std::future_status::ready)
			{
				// collect old jobs
				...
				if (this_id < n_jobs)
				{
					fut = std::async([=, &args...] { return std::make_pair(this_id, func(this_id, args...)); });
					++this_id;
				}
			}
		}
